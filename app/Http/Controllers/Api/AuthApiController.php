...
class AuthApiController extends BaseController {
    ...
    private function getOrdersQuery($request, $status = NULL, $dateRange = false) {
        ...
        $query = Order::leftjoin('restaurants', 'orders.rest_id', '=', 'restaurants.id')
                ...
                ->where(['restaurants.status' => 1]);//'orders.user_id' => $user_id, 
    ...
    public function orderDetails(Request $request) {
        ...
        $exists = Order::leftjoin('restaurants', 'orders.rest_id', '=', 'restaurants.id')
                ->where(['orders.id' => $order_id, 'restaurants.status' => $this->_ACTIVE])
                ->select(['orders.id'])
                ->first();//'orders.user_id' => $user_id, 
    ...
    public function orderCollect(Request $request) {
        ...
        //dump('Order IDs:', $order_ids); // Debugging output

        $response_data = array();
        $message = 'Order Collected Successfully.';

        foreach ($order_ids as $key => $id) {
            //dump('Processing ID:', $id); // Debugging output
            $order = Order::where(['orders.id' => $id, 'orders.status' => $this->_ORDER_READY_TO_SERVE])->first();;
                    
            //$order = Str::replaceArray('?', $order->getBindings(), $order->toSql());dump('Processing query:', $order);

            //$order = $order->first();//leftJoin('restaurants', 'orders.rest_id', '=', 'restaurants.id')->
                    //, 'orders.user_id' => $user_id, 'restaurants.status' => $this->_ACTIVE
            if ($order) {
                $order->collected_time = Carbon::now()->toDateTimeString();//time();
                $order->status = $this->_ORDER_SERVED;
                $order->save();
                $response_data[$order->id] = $this->getOrderDetails($order);
            }
        }
    ...
    private function response_with_countdata(Request $request, $response_data, $message) {
        ...
        $data = $response_data;
        $data['counts_data'] = $counts_data['counts_data'];
