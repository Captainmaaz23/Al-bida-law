...
class OrderController extends MainController {
    ...
    private function commonListing($list_type) {
        ...
        $export_route = 'exports-'.$list_type.'-listing';
        switch($list_type){
            case 'open':
            ...
            default:
                ...
                $export_route = 'exports-all-listing';
                break;            
        }
        ...
        return view($this->views_path . '.listing', compact("recordsExists", "export_route", "list_title", "filter_status", "filter_array", "restaurants_array", "users_array", "tables_array"));
    }
    ...
    private function applyFilters($query, Request $request) {
        if ($request->has('rest_id') && $request->get('rest_id') != -1 && !empty($request->get('rest_id'))) {
            $query->where('orders.rest_id', $request->get('rest_id'));
        }

        if ($request->has('order_no') && $request->get('order_no') != -1 && !empty($request->get('order_no'))) {
            $query->where('orders.order_no', 'LIKE', '%' . $request->get('order_no') . '%');
        }
        ...
        if ($request->has('table_id') && $request->get('table_id') != -1 && !empty($request->get('table_id'))) {
            $query->where('orders.table_id', $request->get('table_id'));
        }

        if ($request->has('order_value') && $request->get('order_value') != -1 && !empty($request->get('order_value'))) {
            $query->where('orders.order_value', '>=', $request->get('order_value'));
        }
