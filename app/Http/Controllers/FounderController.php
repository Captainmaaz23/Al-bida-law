...
class FounderController extends Controller
{
    ...
    private function commonListing($list_type) {
        ...
        switch($list_type) {
            case 'active':
                $filter_status = 1;
                $recordsExists = FounderMessage::where('status', 1)->exists(); // TRUE if active records exist
                $list_title = 'Active '.$list_title;
                break;
    
            case 'inactive':
                $filter_status = 0;
                $recordsExists = FounderMessage::where('status', 0)->exists(); // TRUE if inactive records exist
                $list_title = 'Inactive '.$list_title;
                break;
    
            default:
                $recordsExists = FounderMessage::exists(); // TRUE if any record exists
                $list_title = 'All '.$list_title;
                break;            
        }
    
        return view('general_settings.founder.index', compact("recordsExists", "list_title", "filter_status", "filter_array"));
    }
    ...
    {
        $Records =  FounderMessage::select(['id', 'founder_name','designation','image']);
    
        $response = Datatables::of($Records)
            ->filter(function ($query) use ($request) {
                if ($request->has('founder_name') && !is_null($request->founder_name)) {
                    $query->where('founder_name', 'like', "%{$request->founder_name}%");
                }
            })            
            ->addColumn('founder_name', fn($record) => $record->founder_name)
            ->addColumn('designation', fn($record) => $record->designation)
            ->addColumn('image', function ($record) {
                if ($record->image) {
                    $imageUrl = url('public/uploads/founders/' . $record->image); // Use url() instead of asset()
            ...
            ->rawColumns(['founder_name','designation','image','action'])
    ...
    public function getActionButtons($record) {
        ...
        if ($Auth_User->can($this->view_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('founder.show', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-eye"></i></a>';
        }

        if ($Auth_User->can($this->edit_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('founder.edit', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-edit"></i></a>';
        }
        $action_buttons .= "</div>";
    ...
    {
        return view('general_settings.founder.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $uploadDir = public_path('/uploads/founders');
            $file->move($uploadDir, $fileName);
            $relativePath = $fileName;
        }
    
        $founder = new FounderMessage();
        $founder->arabic_name = $request->arabic_name;
        $founder->arabic_designation = $request->arabic_designation;
        $founder->arabic_message = $request->arabic_message;
        $founder->fill($data);
        $founder->image       = $relativePath;
        $founder->created_by  = Auth::id();
        $founder->updated_by  = Auth::id();
        $founder->save();
    ...
    {
        $founder = FounderMessage::find($id);
        return view('general_settings.founder.show',compact('founder'));
    }
    ...
    {
        $founder = FounderMessage::find($id);
        return view('general_settings.founder.edit',compact('founder'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $relativePath = null;
        $founder = FounderMessage::findOrFail($id);
    
        if ($request->hasFile('image')) {
            if ($founder->image && file_exists(public_path('uploads/founder'.$founder->image))) {
