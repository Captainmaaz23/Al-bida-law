...
class CaseStudyController extends Controller
{
    ...
    private function commonListing($list_type) {
        if (!Auth::user()->can($this->list_permission)) {
            ...
            return redirect()->route('case-study.index'); // Adjust route if needed
        }
        ...
        switch($list_type) {
            case 'active':
                $filter_status = 1;
                $recordsExists = CaseStudy::where('status', 1)->exists(); // TRUE if active records exist
                $list_title = 'Active '.$list_title;
                break;
    
            case 'inactive':
                $filter_status = 0;
                $recordsExists = CaseStudy::where('status', 0)->exists(); // TRUE if inactive records exist
                $list_title = 'Inactive '.$list_title;
                break;
    
            default:
                $recordsExists = CaseStudy::exists(); // TRUE if any record exists
                $list_title = 'All '.$list_title;
                break;            
        }
    
        return view('general_settings.case-study.index', compact("recordsExists", "list_title", "filter_status", "filter_array"));
    }
    ...
    {
        $Records =  CaseStudy::select(['id', 'title','image','client','status']);
        ...
        $response = Datatables::of($Records)
            ...
            ->addColumn('client', fn($record) => $record->client)
            ->addColumn('image', function ($record) {
                if ($record->image) {
                    $imageUrl = url('public/uploads/case-study/' . $record->image); // Use url() instead of asset()
            ...
            ->rawColumns(['title','image','client','action'])
    ...
    public function getActionButtons($record) {
        ...
        if ($Auth_User->can($this->view_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('case-study.show', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-eye"></i></a>';
        }

        if ($Auth_User->can($this->edit_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('case-study.edit', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-edit"></i></a>';
        }
        $action_buttons .= "</div>";
    ...
    {
        return view('general_settings.case-study.create');
    }
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'attorny'      => 'required|string|max:255',   
            'client'       => 'required|string|max:255',
            'started_date' => 'required|date',
            'end_date'     => 'nullable|date|after_or_equal:started_date',
            'decision'     => 'nullable|string',
            'status'       => 'nullable|in:pending,won,lost,dismissed', 
        ]);
        $relativePath = null;   
        ...
        if ($request->hasFile('image')) {
            ...
            $uploadDir   = public_path('uploads/case-study');
        ...
        $casestudy->arabic_title = $request->arabic_title;
        $casestudy->arabic_attorny = $request->arabic_attorny;
        $casestudy->arabic_client = $request->arabic_client;
        $casestudy->arabic_decision = $request->arabic_decision;
        $casestudy->fill($data);
        $casestudy->image       = $relativePath;      
    ...
    public function show(string $id)
    {
        ...
        return view('general_settings.case-study.show',compact('case_study'));
    }
    ...
    {
        $case_study = CaseStudy::find($id);
        return view('general_settings.case-study.edit',compact('case_study'));
    }
