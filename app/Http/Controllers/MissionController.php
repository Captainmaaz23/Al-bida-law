...
class MissionController extends Controller
{
    ...
    private function commonListing($list_type) {
        ...
        switch($list_type) {
            case 'active':
                $filter_status = 1;
                $recordsExists = Mission::where('status', 1)->exists(); // TRUE if active records exist
                $list_title = 'Active '.$list_title;
                break;
    
            case 'inactive':
                $filter_status = 0;
                $recordsExists = Mission::where('status', 0)->exists(); // TRUE if inactive records exist
                $list_title = 'Inactive '.$list_title;
                break;
    
            default:
                $recordsExists = Mission::exists(); // TRUE if any record exists
                $list_title = 'All '.$list_title;
                break;            
        }
    
        return view('general_settings.missions.index', compact("recordsExists", "list_title", "filter_status", "filter_array"));
    }
    ...
    {
        $Records =  Mission::select(['id', 'title','arabic_title']);
        ...
        $response = Datatables::of($Records)
            ...
            ->addColumn('arabic_title', fn($record) => $record->arabic_title)  
    ...
    public function getActionButtons($record) {
        ...
        if ($Auth_User->can($this->view_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('mission.show', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-eye"></i></a>';
        }

        if ($Auth_User->can($this->edit_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('mission.edit', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-edit"></i></a>';
        }
        $action_buttons .= "</div>";
    ...
    public function makeActive($id) {
        ...
        Mission::where('id', $id)->update(['status' => 1, 'updated_by' => $Auth_User->id]);
    ...
    public function makeInActive($id) {
        ...
        Mission::where('id', $id)->update(['status' => 0, 'updated_by' => $Auth_User->id]);
    ...
    {
        return view('general_settings.missions.create');
    }
    ...
    {
        $certificate = Mission::find($id);
        return view('general_settings.missions.show',compact('certificate'));
    }
    ...
    {
        $certificate = Mission::find($id);
        return view('general_settings.missions.edit',compact('certificate'));
    }
