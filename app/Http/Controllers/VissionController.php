...
class VissionController extends Controller
{
    ...
    private function commonListing($list_type) {
        ...
        switch($list_type) {
            case 'active':
                $filter_status = 1;
                $recordsExists = Vission::where('status', 1)->exists(); // TRUE if active records exist
                $list_title = 'Active '.$list_title;
                break;
    
            case 'inactive':
                $filter_status = 0;
                $recordsExists = Vission::where('status', 0)->exists(); // TRUE if inactive records exist
                $list_title = 'Inactive '.$list_title;
                break;
    
            default:
                $recordsExists = Vission::exists(); // TRUE if any record exists
                $list_title = 'All '.$list_title;
                break;            
        }
    
        return view('general_settings.vissions.index', compact("recordsExists", "list_title", "filter_status", "filter_array"));
    }
    ...
    public function datatable(Request $request) {
        ...
        Flash::error($this->list_permission_error_message);
        return redirect()->route('vission.index');
    }

    public function service_datatable(Request $request)
    {
        $Records =  Vission::select(['id', 'title','arabic_title']);
        ...
        $response = Datatables::of($Records)
            ...
            ->rawColumns(['title','action','arabic_title'])
    ...
    public function getActionButtons($record) {
        ...
        if ($Auth_User->can($this->view_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('vission.show', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-eye"></i></a>';
        }

        if ($Auth_User->can($this->edit_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('vission.edit', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-edit"></i></a>';
        }
        $action_buttons .= "</div>";
    ...
    public function makeActive($id) {
        ...
        Vission::where('id', $id)->update(['status' => 1, 'updated_by' => $Auth_User->id]);
    ...
    public function makeInActive($id) {
        ...
        Vission::where('id', $id)->update(['status' => 0, 'updated_by' => $Auth_User->id]);
    ...
    {
        return view('general_settings.vissions.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $mission->arabic_title = $request->arabic_title;
        $mission->arabic_description = $request->arabic_description;
    ...
    {
        $certificate = Vission::find($id);
        return view('general_settings.vissions.show',compact('certificate'));
    }
    ...
    {
        $certificate = Vission::find($id);
        return view('general_settings.vissions.edit',compact('certificate'));
    }
