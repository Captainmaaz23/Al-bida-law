...
class AboutController extends Controller
{
    ...
    private function commonListing($list_type) {
        if (!Auth::user()->can($this->list_permission)) {
            ...
            return redirect()->route('about.index'); // Adjust route if needed
        }
        ...
        switch($list_type) {
            case 'active':
                $filter_status = 1;
                $recordsExists = About::where('status', 1)->exists(); // TRUE if active records exist
                $list_title = 'Active '.$list_title;
                break;
    
            case 'inactive':
                $filter_status = 0;
                $recordsExists = About::where('status', 0)->exists(); // TRUE if inactive records exist
                $list_title = 'Inactive '.$list_title;
                break;
    
            default:
                $recordsExists = About::exists(); // TRUE if any record exists
                $list_title = 'All '.$list_title;
                break;            
        }
    
        return view('restaurants.abouts.index', compact("recordsExists", "list_title", "filter_status", "filter_array"));
    }
    ...
    {
        $Records = About::select(['id', 'title','image_title','image','status']);
        ...
        $response = Datatables::of($Records)
            ...
            ->addColumn('image_title', fn($record) => $record->image_title)
            ->addColumn('image', function ($record) {
                if ($record->image) {
                    $imageUrl = url('public/uploads/abouts/' . $record->image); // Use url() instead of asset()
            ...
            ->rawColumns(['title','image_title','image','action','status'])
    ...
    public function getActionButtons($record) {
        ...
        if ($Auth_User->can($this->view_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('about.show', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-eye"></i></a>';
        }

        if ($Auth_User->can($this->edit_permission) || $Auth_User->can('all')) {
            $action_buttons .= '<a href="' . route('about.edit', $record->id) . '" class="btn btn-outline-primary"><i class="fa fa-edit"></i></a>';
        }
        $action_buttons .= "</div>";
    ...
    {
        return view('restaurants.abouts.create');
    }
    ...
    {
        $about = About::find($id);
        return view('restaurants.abouts.show',compact('about'));
    }
    ...
    {
        $about = About::find($id);
        return view('restaurants.abouts.edit',compact('about'));
    }
    ...
    public function update(Request $request, string $id)
{
    ...
    if ($request->hasFile('image')) {
        ...
        $oldImagePath = public_path('uploads/abouts/' . $about->image);
    ...
    // Update all other fields (nullable-safe)
    $about->title = $request->input('title');
    $about->arabic_title = $request->input('arabic_title');
