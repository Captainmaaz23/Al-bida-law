    ...
    <div class="content">
        ...
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            ...
            @if(Auth::user()->can('users-add') || Auth::user()->can('all'))
                <form method="post" action="{{ route($export_route) }}">
                    @csrf
                    <button type="submit" class="btn btn-primary btn-add-new pull-right">
                        <i class="fa fa-file-excel-o" aria-hidden="true"></i> Export All
                    </button>
                    @if($AUTH_USER->rest_id == 0)
                    <input type="hidden" name="rest_id" id="s_rest_id_export" value="">
                    @endif
                    <input type="hidden" name="order_no" id="s_order_no_export" value="">
                    <input type="hidden" name="user_id" id="s_user_id_export" value="">
                    <input type="hidden" name="table_id" id="s_table_id_export" value="">
                    <input type="hidden" name="order_value" id="s_order_value_export" value="">
                    <input type="hidden" name="status" id="s_status_export" value="">
                </form>
            @endif
        </div>
    ...
    jQuery(document).ready(function () {
        @if($recordsExists == 1)
            ...
            $('#s_rest_id, #s_order_no, #s_user_id, #s_table_id, #s_order_value, #s_status').on('change keyup', function (e) {
                @if($AUTH_USER->rest_id == 0)
                    $('#s_rest_id_export').val($('#s_rest_id').val());
                @endif                        
                $('#s_order_no_export').val($('#s_order_no').val());                        
                $('#s_user_id_export').val($('#s_user_id').val());
                $('#s_table_id_export').val($('#s_table_id').val());
                $('#s_order_value_export').val($('#s_order_value').val());
                $('#s_status_export').val($('#s_status').val());
                oTable.draw();
                e.preventDefault();
            });
        @endif
