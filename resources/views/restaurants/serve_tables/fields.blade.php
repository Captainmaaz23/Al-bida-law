    ...
    <div class="col-sm-6">
        @if (Auth::user()->rest_id == 0)
            <div class="form-group row mt-3">
                <label for="rest_id" class="col-sm-4">Restaurant:</label>
                <div class="col-sm-8">
                    <select name="rest_id" class="form-control js-select2 form-select rest_select2" {{ isset($Model_Data->rest_id) ? 'disabled' : '' }}>
                        <option value="" {{ !isset($Model_Data->rest_id) ? 'disabled selected' : 'selected' }}>Select</option>
                        @foreach ($restaurants_array as $value => $label)
                            <option value="{{ $value }}" {{ (isset($Model_Data->rest_id) && $value == $Model_Data->rest_id) ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="form-group row mt-3">
            ...
            <div class="col-sm-8">
                <input type="text" name="title" class="form-control" value="{{ (isset($Model_Data->title)) ? $Model_Data->title : '' }}">
            </div>
        </div>

        <div class="form-group row mt-3">
            <label for="file_upload" class="col-sm-4">Image:</label>
            <div class="col-sm-8">
        ...
        <div class="form-group row mt-3">
            <label for="availability" class="col-sm-4">Availability:</label>
            <div class="col-sm-8">
